...
public class ScheduleView extends View implements OnTouchListener {
	...
	private static final int sidewidte = 80;//左边，上面bar的宽度
	...
	private String[] daytimes;//上课时间
	private List<String> _datelist;//日期
	...
	// 颜色
	public static final int barBg1 = Color.argb(255, 80, 150, 220);
	...
	public ScheduleView(Context context, AttributeSet attrs) {
		...
		daytimes = context.getResources().getStringArray(R.array.daytimes);
		mPaint = new Paint();
	...
	private void printLeftBar(Canvas canvas) {
		...
		// 画第一个文字
		mPaint.setTextSize(30);
		canvas.drawText("1", sidewidte / 2 - tw1, startY+sidewidte+60, mPaint);
		mPaint.setTextSize(20);
		canvas.drawText(daytimes[0], sidewidte /4, startY +sidewidte+90, mPaint);
		for (int i = 2; i < classTotal + 1; i++) {
			...
			canvas.drawText(i + "", sidewidte / 2 - tw / 2, startY + sidewidte
					+ eachBoxH * (i - 1) + 60, mPaint);
			mPaint.setTextSize(20);
			canvas.drawText(daytimes[i-1], sidewidte /4, startY +sidewidte+eachBoxH*(i-1)+90, mPaint);
		}
		// =========左上角正方形============
		mPaint.setColor(barBg);
	...
	private void printTopBar(Canvas canvas) {
		...
		Rect textBounds = new Rect();
			...
			mPaint.getTextBounds(_datelist.get(0), 0,_datelist.get(0).length(), textBounds);

		int textHeight = textBounds.bottom - textBounds.top;
		int textWidth = textBounds.right - textBounds.left;	
		for (int i = 1; i < dayTotal + 1; i++) {

			if (_datelist.get(i-1).equals(NowDate)) {
				mPaint.setColor(barBg1);
				// 画边框线
				...
				canvas.drawText(weekdays[i-1], startX + sidewidte + eachBoxW
						* (i - 1) + eachBoxW / 2 - textWidth / 2,sidewidte/3, mPaint);
					canvas.drawText(_datelist.get(i-1), startX + sidewidte + eachBoxW
							* (i - 1) + eachBoxW / 2 - textWidth / 2, sidewidte / 2 + textHeight , mPaint);
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (startX + dx + eachBoxW * dayTotal + sidewidte >= getWidth()) {
